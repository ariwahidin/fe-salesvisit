...
import { ordersApi, supervisorApi, formatDate } from '@/lib/api'
...
export default function OrdersPage() {
  ...
  const load = async () => {
    ...
    try {
      const params: any = {}
      if (isAdmin && filterSupervisor) params.supervisor_id = filterSupervisor

      const [oRes] = await Promise.all([
        ordersApi.list(params),
        // Load supervisors sekali saja
        ...(isAdmin && supervisors.length === 0
          ? [supervisorApi.getSupervisors().then(r => setSupervisors(r.data || []))]
          : []
        ),
      ])
      setOrders(oRes.data || [])
  ...
  useEffect(() => { load() }, [filterSupervisor])
      ...
      <div className="animate-fade-in">
        ...
        <div className="p-4 pb-0">
          <div className="flex items-center justify-between mb-1">
            <h1 className="font-extrabold text-surface-900 text-lg">Manajemen Order</h1>
            {isAdmin && supervisors.length > 0 && (
          ...
          <p className="text-xs text-surface-500 mb-3">
            {counts.all} order ·{' '}
            <span className="text-amber-600 font-semibold">{counts.submitted} perlu ditinjau</span>
          </p>

          {/* Filter supervisor (admin only) */}
          {isAdmin && showFilters && supervisors.length > 0 && (
            <div className="mb-3">
